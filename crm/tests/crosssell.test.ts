...
function setupDb() {
  ...
  testDb
    .prepare(
      `INSERT INTO cuenta (id, nombre, tipo, vertical, ae_id, años_relacion, es_fundador) VALUES ('c1', 'Acme Corp', 'directo', 'Consumo', 'ae-001', 5, 1)`,
    )
    .run();
  testDb
    .prepare(
      `INSERT INTO cuenta (id, nombre, tipo, vertical, ae_id, años_relacion, es_fundador) VALUES ('c2', 'Beta Inc', 'directo', 'Consumo', 'ae-002', 3, 0)`,
    )
    .run();
  testDb
    .prepare(
      `INSERT INTO cuenta (id, nombre, tipo, vertical, ae_id, años_relacion, es_fundador) VALUES ('c3', 'Gamma SA', 'agencia', 'Consumo', 'ae-001', 2, 0)`,
    )
    .run();
  // Different vertical
  testDb
    .prepare(
      `INSERT INTO cuenta (id, nombre, tipo, vertical, ae_id, años_relacion) VALUES ('c4', 'Delta Tech', 'directo', 'Tecnologia', 'ae-002', 1)`,
    )
    .run();
}
...
function seedCompletedProposals() {
  ...
  testDb
    ...
    VALUES ('p1', 'c1', 'ae-001', 'Acme Verano', 5000000, 'estacional', 'completada', ?, ?)`,
    )
    .run(recent, now);
  testDb
    .prepare(
      `INSERT INTO propuesta (id, cuenta_id, ae_id, titulo, valor_estimado, tipo_oportunidad, etapa, fecha_creacion, fecha_ultima_actividad)
    VALUES ('p2', 'c1', 'ae-001', 'Acme Refuerzo', 3000000, 'reforzamiento', 'completada', ?, ?)`,
    )
    .run(recent, now);
  ...
  testDb
    .prepare(
      `INSERT INTO propuesta (id, cuenta_id, ae_id, titulo, valor_estimado, tipo_oportunidad, etapa, fecha_creacion, fecha_ultima_actividad)
    VALUES ('p3', 'c2', 'ae-002', 'Beta Navidad', 8000000, 'estacional', 'completada', ?, ?)`,
    )
    .run(recent, now);
  testDb
    .prepare(
      `INSERT INTO propuesta (id, cuenta_id, ae_id, titulo, valor_estimado, tipo_oportunidad, etapa, fecha_creacion, fecha_ultima_actividad)
    VALUES ('p4', 'c2', 'ae-002', 'Beta Copa', 12000000, 'tentpole', 'completada', ?, ?)`,
    )
    .run(recent, now);
  testDb
    .prepare(
      `INSERT INTO propuesta (id, cuenta_id, ae_id, titulo, valor_estimado, tipo_oportunidad, etapa, fecha_creacion, fecha_ultima_actividad)
    VALUES ('p5', 'c2', 'ae-002', 'Beta Lanzamiento', 4000000, 'lanzamiento', 'completada', ?, ?)`,
    )
    .run(recent, now);
  ...
  testDb
    .prepare(
      `INSERT INTO propuesta (id, cuenta_id, ae_id, titulo, valor_estimado, tipo_oportunidad, etapa, fecha_creacion, fecha_ultima_actividad)
    VALUES ('p6', 'c3', 'ae-001', 'Gamma Evento', 6000000, 'tentpole', 'completada', ?, ?)`,
    )
    .run(recent, now);
}
...
function seedActivities() {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    testDb
      ...
      VALUES (?, 'ae-001', 'c1', 'llamada', 'Seguimiento', 'positivo', ?)`,
      )
      .run(`act-${i}`, date);
  }
  ...
  testDb
    .prepare(
      `INSERT INTO actividad (id, ae_id, cuenta_id, tipo, resumen, sentimiento, fecha)
    VALUES ('act-old', 'ae-002', 'c4', 'email', 'Contacto inicial', 'neutral', ?)`,
    )
    .run(new Date(now.getTime() - 45 * 86400000).toISOString());
}
...
function seedEvents() {
  ...
  testDb
    .prepare(
      `INSERT INTO crm_events (id, nombre, tipo, fecha_inicio, meta_ingresos, ingresos_actual, prioridad)
    VALUES ('ev1', 'Copa del Mundo', 'deportivo', ?, 50000000, 30000000, 'alta')`,
    )
    .run(futureDate);
}
...
describe("recomendar_crosssell", () => {
  ...
  it("returns error for unknown account", () => {
    ...
    const result = JSON.parse(
      recomendar_crosssell({ cuenta_nombre: "Nonexistent" }, ctx),
    );
  ...
  it("returns account context info", () => {
    ...
    const ctx = makeCtx("vp", "vp-001");
    const result = JSON.parse(
      recomendar_crosssell({ cuenta_nombre: "Acme" }, ctx),
    );
  ...
  it("resolves an advertiser-superset name the bare LIKE would miss (findCuentaId fallback)", () => {
    // "Acme Corp" ⊊ "Acme Corp de México": the one-directional LIKE misses it,
    // but the hardened findCuentaId fallback (whole-token subset) resolves it —
    // a known in-cartera account is no longer reported as unknown.
    seedCompletedProposals();
    const ctx = makeCtx("vp", "vp-001");
    const result = JSON.parse(
      recomendar_crosssell({ cuenta_nombre: "Acme Corp de México" }, ctx),
    );
  ...
  it("identifies tipo_oportunidad gaps from peers", () => {
    ...
    const tipoRecs = result.recomendaciones.filter(
      (r: any) => r.tipo === "tipo_oportunidad",
    );
  ...
  it("does not recommend tipos already purchased", () => {
    ...
    expect(titulos.some((t: string) => t.includes("reforzamiento"))).toBe(
      false,
    );
  });
  ...
  it("does not recommend tipos already in flight", () => {
    ...
    testDb
      .prepare(
        `INSERT INTO propuesta (id, cuenta_id, ae_id, titulo, valor_estimado, tipo_oportunidad, etapa, fecha_creacion, fecha_ultima_actividad)
      VALUES ('p-active', 'c1', 'ae-001', 'Acme Tentpole', 10000000, 'tentpole', 'en_discusion', datetime('now'), datetime('now'))`,
      )
      .run();
  ...
  it("includes event recommendations when events exist", () => {
    ...
    const eventRecs = result.recomendaciones.filter(
      (r: any) => r.tipo === "evento",
    );
  ...
  it("AE only sees own accounts", () => {
    ...
    const result2 = JSON.parse(
      recomendar_crosssell({ cuenta_nombre: "Beta" }, ctx),
    );
  ...
  it("respects limite parameter", () => {
    ...
    const ctx = makeCtx("vp", "vp-001");
    const result = JSON.parse(
      recomendar_crosssell({ cuenta_nombre: "Acme", limite: 2 }, ctx),
    );
    expect(result.recomendaciones.length).toBeLessThanOrEqual(2);
    expect(result.total_recomendaciones).toBeGreaterThanOrEqual(
  ...
  it("detects reactivation opportunity for stale accounts", () => {
    ...
    testDb
      .prepare(
        `INSERT INTO propuesta (id, cuenta_id, ae_id, titulo, valor_estimado, tipo_oportunidad, etapa, fecha_creacion, fecha_ultima_actividad)
      VALUES ('p-delta', 'c4', 'ae-002', 'Delta Campaign', 3000000, 'estacional', 'completada', datetime('now', '-60 days'), datetime('now', '-30 days'))`,
      )
      .run();
    ...
    const ctx = makeCtx("vp", "vp-001");
    const result = JSON.parse(
      recomendar_crosssell({ cuenta_nombre: "Delta" }, ctx),
    );
    const reactivacion = result.recomendaciones.filter(
      (r: any) => r.tipo === "reactivacion",
    );

...
"Carlos Ruiz",gerente,+5222222222`;
  ...
  it("parses CSV with all optional columns", () => {
    const csv = `name,role,phone,email,calendar_id,manager_name
"VP Name",vp,+521000,vp@co.com,vp@cal.com,`;

    const members = parseCsv(csv);
    expect(members[0].email).toBe("vp@co.com");
    expect(members[0].calendar_id).toBe("vp@cal.com");
...
"Test",+521000`;
    ...
    expect(() => parseCsv(csv)).toThrow(
      "CSV must have name, role, and phone columns",
    );
  });
...
describe("parseJson", () => {
  ...
  it("throws on invalid role", () => {
    const json = JSON.stringify([
      { name: "Test", role: "intern", phone: "+521000" },
    ]);
  ...
  it("normalizes role to lowercase", () => {
    const json = JSON.stringify([
      { name: "Test", role: "VP", phone: "+521000" },
    ]);
    const members = parseJson(json);
...
describe("generateGroupFolder", () => {
  ...
  it("strips accents", () => {
    expect(generateGroupFolder("José García", "gerente")).toBe(
      "gerente-jose-garcia",
    );
  });
  ...
  it("handles multi-word names", () => {
    expect(generateGroupFolder("Ana María De La Cruz", "director")).toBe(
      "director-ana-cruz",
    );
  });
  ...
  it("handles extra whitespace", () => {
    expect(generateGroupFolder("  María   López  ", "ae")).toBe(
      "ae-maria-lopez",
    );
  });
});
...
describe("registerTeam", () => {
  it("inserts personas into database", () => {
    ...
    const rows = testDb
      .prepare("SELECT * FROM persona ORDER BY rol")
      .all() as any[];
  ...
  it("resolves manager hierarchy", () => {
    const members = [
      ...
      },
      {
        ...
        phone: "+523000",
        manager_name: "Dir Ana",
      },
      {
        name: "AE María",
        role: "ae" as const,
        phone: "+524000",
        manager_name: "Ger Miguel",
      },
    ];
    ...
    const ae = testDb
      .prepare("SELECT reporta_a FROM persona WHERE nombre = 'AE María'")
      .get() as any;
    const ger = testDb
      .prepare("SELECT reporta_a FROM persona WHERE nombre = 'Ger Miguel'")
      .get() as any;
    const dir = testDb
      .prepare("SELECT reporta_a FROM persona WHERE nombre = 'Dir Ana'")
      .get() as any;
    const vp = testDb
      .prepare("SELECT reporta_a FROM persona WHERE nombre = 'VP Roberto'")
      .get() as any;
  ...
  it("generates correct group folders", () => {
    ...
    const row = testDb
      .prepare("SELECT whatsapp_group_folder FROM persona")
      .get() as any;
  ...
  it("stores email and phone", () => {
    ...
    const row = testDb
      .prepare("SELECT telefono, email, calendar_id FROM persona")
      .get() as any;
    expect(row.telefono).toBe("+521111");
    expect(row.email).toBe("test@co.com");
    expect(row.calendar_id).toBe("cal@co.com");
  });
  ...
  it("handles duplicate registration (INSERT OR IGNORE)", () => {
    ...
    registerTeam(members);
    const count1 = (
      testDb.prepare("SELECT COUNT(*) as c FROM persona").get() as any
    ).c;
